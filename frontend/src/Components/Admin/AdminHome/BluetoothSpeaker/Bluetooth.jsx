...
function Bluetooth() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingProduct) {
        await axios.put(`http://localhost:5000/api/admin/bluetoothspeakers/${editingProduct._id}`, formData);
        alert("Product updated successfully");
      } else {
        await axios.post("http://localhost:5000/api/admin/bluetoothspeakers", formData);
  ...
  const handleDelete = async (productId) => {
    try {
      await axios.delete(`http://localhost:5000/api/admin/bluetoothspeakers/${productId}`);
